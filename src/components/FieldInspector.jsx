...
export default function FieldInspector({ field, index, total, onChange, onDuplicate, onDelete, onMove }) {
  ...
  return (
    <div className="inspector-panel">
      ...
      {field.type === 'consent' ? <><label className="studio-control"><span>체크박스 문구</span><textarea rows="3" value={field.consentText || ''} maxLength="500" onChange={(event) => patch({ consentText: event.target.value })} placeholder="내용을 확인했으며 동의합니다." /></label><label className="studio-control"><span>안내 링크 주소 · 선택</span><input type="url" value={field.consentLinkUrl || ''} onChange={(event) => patch({ consentLinkUrl: event.target.value })} placeholder="https://example.com/privacy" /></label><label className="studio-control"><span>링크 문구</span><input value={field.consentLinkLabel || ''} maxLength="120" disabled={!field.consentLinkUrl} onChange={(event) => patch({ consentLinkLabel: event.target.value })} placeholder={field.consentLinkUrl ? '개인정보 처리방침 보기' : '주소를 먼저 입력하세요'} /></label></> : null}
