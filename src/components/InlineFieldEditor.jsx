...
export default function InlineFieldEditor({ field, index, total, selected, accent, onSelect, onChange, onDuplicate, onDelete, onMove }) {
  ...
  return (
    ...
    >
      ...
      {selected ? <div className="inline-field-controls">
        ...
        <div className="inline-field-actions">
          <button type="button" onClick={() => onMove(-1)} disabled={index === 0} aria-label="위로 이동"><ArrowUp /></button>
          <button type="button" onClick={() => onMove(1)} disabled={index === total - 1} aria-label="아래로 이동"><ArrowDown /></button>
          <button type="button" onClick={onDuplicate} aria-label="질문 복제"><Copy /></button>
          <button className="danger" type="button" onClick={onDelete} aria-label="질문 삭제"><Trash /></button>
        </div>
      </div> : null}
    </article>
  )
