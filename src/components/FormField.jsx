...
export default function FormField({
  ...
  return (
    <fieldset className={`render-field ${error ? 'field-invalid' : ''}`} style={{ '--field-accent': accent }}>
      <legend className={hidePrompt || isConsent ? 'visually-hidden' : ''}>{field.label}{!hidePrompt && field.required && requiredLabel ? <span>{requiredLabel}</span> : null}</legend>
      ...
      {field.type === 'consent' ? <label className="consent-row"><input disabled={preview} type="checkbox" checked={value === '동의'} onChange={(event) => set(event.target.checked ? '동의' : '')} /><span className="choice-indicator"><Check weight="bold" /></span><span className="consent-copy">{field.label || consentLabel}</span>{field.required && requiredLabel ? <small>{requiredLabel}</small> : null}</label> : null}
