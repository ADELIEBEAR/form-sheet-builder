...
export default function AllResponses() {
  ...
  const exportRows = [['제출 시각', '폼', 'DB 판정', '판정 사유', '응답 미리보기', '시트 상태'], ...filtered.map((submission) => [new Date(submission.submittedAt).toLocaleString('ko-KR'), projectMap[submission.projectId]?.title || '삭제된 폼', qualityLabel(submission.qualityStatus), qualityReasonText(submission.qualityReasons), answerPreview(submission.answers), statusLabel[submission.sheetSyncStatus] || submission.sheetSyncStatus])]
      ...
      <main className="workspace-main all-responses-main">
        ...
        {!loading && adminReady ? <>
          <section className="response-overview" aria-label="응답 요약">
            <article><span>전체 응답</span><strong>{accessibleSubmissions.length.toLocaleString()}</strong><small>{projects.length.toLocaleString()}개 폼 · 시트 대기 {pending.toLocaleString()}개{failed ? ` · 실패 ${failed.toLocaleString()}개` : ''}</small></article>
