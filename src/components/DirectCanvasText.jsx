...
export default function DirectCanvasText({ children, value, fallback, minSize, maxSize, label, selected, onSelect, onChange, className = '' }) {
  const resolved = resolveDirectTextStyle(value, fallback)
  ...
  const beginPointerAction = (event, mode) => {
    ...
    const move = (moveEvent) => {
      ...
      if (mode === 'move') {
        patch({ offsetX: Math.round(clamp(start.offsetX + deltaX, -120, 120)), offsetY: Math.round(clamp(start.offsetY + deltaY, -100, 100)) })
        return
      }
      if (mode === 'width') {
        patch({ width: Math.round(clamp(start.width + ((deltaX / hostWidth) * 100), 48, 100)) })
        return
      }
      const delta = (deltaX + deltaY) / 5
      patch({ size: Math.round(clamp(start.size + delta, minSize, maxSize) * 10) / 10 })
    }
    ...
    >
      ...
      {selected ? <>
        <div className="direct-text-toolbar" role="toolbar" aria-label={`${label} 빠른 디자인`} onPointerDown={(event) => event.stopPropagation()}>
          ...
          <button type="button" onClick={() => patch({ size: clamp(resolved.size - 1, minSize, maxSize) })} aria-label={`${label} 글자 줄이기`}><Minus weight="bold" /></button>
          <output aria-label={`${label} 현재 크기`}>{Math.round(resolved.size)}px</output>
          <button type="button" onClick={() => patch({ size: clamp(resolved.size + 1, minSize, maxSize) })} aria-label={`${label} 글자 키우기`}><Plus weight="bold" /></button>
        ...
        <button className="direct-width-handle" type="button" onPointerDown={(event) => beginPointerAction(event, 'width')} onKeyDown={(event) => { if (event.key === 'ArrowLeft' || event.key === 'ArrowRight') { event.preventDefault(); patch({ width: clamp(resolved.width + (event.key === 'ArrowRight' ? 2 : -2), 48, 100) }) } }} aria-label={`${label} 너비 조절`} title="좌우로 드래그해 너비 조절" />
        <button className="direct-size-handle" type="button" onPointerDown={(event) => beginPointerAction(event, 'size')} onKeyDown={(event) => { if (['ArrowUp', 'ArrowRight', 'ArrowDown', 'ArrowLeft'].includes(event.key)) { event.preventDefault(); const grow = event.key === 'ArrowUp' || event.key === 'ArrowRight'; patch({ size: clamp(resolved.size + (grow ? 1 : -1), minSize, maxSize) }) } }} aria-label={`${label} 글자 크기 조절`} title="드래그해 글자 크기 조절" />
      </> : null}
    </div>
  )
}
