...
export default function FormField({
  ...
  return (
    <fieldset className={`render-field ${error ? 'field-invalid' : ''}`} style={{ '--field-accent': accent }}>
      ...
      {field.type === 'consent' ? <div className="consent-answer">{privacyConsent ? <dl className="privacy-consent-notice"><div><dt>수집·이용 목적</dt><dd>{field.consentPurpose}</dd></div><div><dt>수집 항목</dt><dd>{field.consentItems}</dd></div><div><dt>보유·이용 기간</dt><dd>{field.consentRetention}</dd></div><div><dt>동의 거부 권리 및 불이익</dt><dd>{field.consentRefusal}</dd></div></dl> : null}<label className="consent-row"><input disabled={preview} type="checkbox" checked={value === '동의'} onChange={(event) => set(event.target.checked ? '동의' : '')} /><span className="choice-indicator"><Check weight="bold" /></span><span className="consent-copy">{consentText || '동의합니다.'}</span></label>{field.consentLinkUrl ? <a className="consent-link" href={field.consentLinkUrl} target="_blank" rel="noreferrer">{field.consentLinkLabel || '자세히 보기'}</a> : null}</div> : null}
